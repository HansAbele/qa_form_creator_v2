...
async function expectRestrictedFormDenied(page: Page, fallback: "/forms" | "error") {
  ...
  if (fallback === "/forms") {
    ...
    await expect(
      page.getByRole("alert").filter({ hasText: "Unable to load this section" }),
    ).toBeVisible();
  }
}
...
test.describe("RBAC matrix - QA Manager", () => {
  ...
  test("has global controls and can see both campaigns", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Export" })).not.toBeVisible();
...
test.describe("RBAC matrix - standard QA", () => {
  ...
  test("has the daily QA workspace only for the assigned campaign", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Dispositions", exact: true })).toBeVisible();
    await expect(page.getByRole("link", { name: "Export" })).not.toBeVisible();
    ...
    await expect(
      page.getByRole("link", { name: `Edit ${ASSIGNED_FORM}`, exact: true }),
    ).not.toBeVisible();
...
test.describe("RBAC matrix - Supervisor", () => {
  ...
  test("is read-only and remains scoped to the assigned campaign", async ({ page }) => {
    ...
    await expectAssignedCampaignOnly(page);
    await expect(page.getByRole("link", { name: "Evaluate" })).not.toBeVisible();
    await expect(page.getByRole("link", { name: "New Form" })).not.toBeVisible();
