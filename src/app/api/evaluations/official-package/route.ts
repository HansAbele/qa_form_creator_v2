...
import { HapusaPackageError } from "@/server/hapusa-evaluation-package";
import {
  MAX_OFFICIAL_PACKAGE_EVALUATIONS,
  OfficialEvaluationPackageError,
  prepareOfficialEvaluationPackage,
} from "@/server/official-evaluation-package";
import { ParkerDavisPackageError } from "@/server/parker-davis-evaluation-package";
...
const requestSchema = z
  .object({
    responseIds: z
      ...
      .min(1)
      .max(MAX_OFFICIAL_PACKAGE_EVALUATIONS),
  })
  .strict();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const prepared = await prepareOfficialEvaluationPackage(input.responseIds, session.user.id);
    ...
    archive.on("warning", (error) => {
      logger.warn({ err: error, userId: session.user.id }, "Official QA package archive warning");
    });
    archive.on("error", (error) => {
      logger.error({ err: error, userId: session.user.id }, "Official QA package archive failed");
    });
    ...
    if (
      error instanceof OfficialEvaluationPackageError ||
      error instanceof HapusaPackageError ||
      error instanceof ParkerDavisPackageError
    ) {
      const status =
    ...
    logger.error({ err: error, userId: session.user.id }, "Official QA package preparation failed");
    return jsonError(500, "EXPORT_FAILED", "The QA evidence package could not be created.");
  }
}
